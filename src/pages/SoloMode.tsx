...
import { ArrowLeft, Timer, BookOpen, Sparkles } from "lucide-react";
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="mb-6">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-primary text-xs font-semibold mb-3">
            <Sparkles className="w-3.5 h-3.5" /> Solo Modes
          </div>
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight">Solo</h1>
          <p className="text-muted-foreground">Practice or go fast in Speed Drive.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          <GlowTile title="Practice" subtitle="Your own pace" icon={<BookOpen className="w-7 h-7"/>} onClick={() => navigate("/modes/solo/practice")} gradient="bg-gradient-to-r from-indigo-400/40 to-purple-400/40"/>
          <GlowTile title="Speed Drive" subtitle="10 Qs • 30s each" icon={<Timer className="w-7 h-7"/>} onClick={() => navigate("/modes/solo/speed")} gradient="bg-gradient-to-r from-emerald-400/40 to-cyan-400/40"/>
        </div>
      </div>
    </div>
  );
